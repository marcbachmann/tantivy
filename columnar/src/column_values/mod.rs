...
pub trait ColumnValues<T: PartialOrd = u64>: Send + Sync + DowncastSync {
    ...
    /// Minimum number of rows a [`Self::get_range`] call must cover before its
    /// batch decode beats a plain `get_val` loop over the same rows.
    ///
    /// The crossover is a property of the column -- it moves with the codec's
    /// `get_val` cost and with whether a SIMD block kernel is available -- so
    /// callers read it here instead of hardcoding a length. `usize::MAX` (the
    /// default) means "never"; columns that decode in batches override this.
...
mod tests {
    ...
    fn specializations_survive_arc() {
        ...
        assert_eq!(out_opt, [Some(22), Some(22)], "get_vals_opt is not forwarded");

        let mut out = [0u64; 2];
        column.get_range(0, &mut out);
        assert_eq!(out, [33, 33], "get_range is not forwarded");

        assert_eq!(column.min_batch_rows(), 44, "min_batch_rows is not forwarded");
        assert_eq!(column.iter().collect::<Vec<_>>(), vec![55], "iter is not forwarded");

        let mut hits = Vec::new();
        column.get_row_ids_for_value_range(0..=100, 0..10, &mut hits);
        assert_eq!(hits, vec![66], "get_row_ids_for_value_range is not forwarded");
    }
}
